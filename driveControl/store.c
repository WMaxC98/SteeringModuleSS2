...
void store_init(Store* me) {
    if (DATAEE_ReadByte(EE_INIT) != 0xA5) { // EEPROM never initialised
        ...
        DATAEE_WriteByte(EE_CENTER_LL, 5); // speed each 50ms
        DATAEE_WriteByte(EE_CENTER_L, 5); // speed each 50ms
        DATAEE_WriteByte(EE_CENTER_H, 5); // speed each 50ms
        DATAEE_WriteByte(EE_CENTER_HH, 5); // speed each 50ms
