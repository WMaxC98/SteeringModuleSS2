...
void storeSetup(CommControl* me, uCAN_MSG* msg);
void sendSpeedFrame(CommControl* me);
void setPower(CommControl* me);
void setBrake(CommControl* me);
...
void commControl_startBehaviour(CommControl* me) {
    ...
    POST(me, &commControl_processEvent, evSInit, 0, 0);
...
_Bool commControl_processEvent(Event* ev) {
    ...
    SpeedSMState oldSSMState = me->speedSM_State;
    ...
    if (processed == 0)
    {
        switch (me->speedSM_State) {
            case ST_SSMINIT:
                if (Event_getId(ev) == evSInit) {
                    me->speedSM_State = ST_SSMWAIT;
                }
                break;
            case ST_SSMWAIT:
                if (Event_getId(ev) == evSTM) {
                    me->speedSM_State = ST_SSMSPEED;
                }
                break;
            case ST_SSMSPEED:
        ...
        if (oldSSMState != me->speedSM_State) {
            processed = 1;
            switch (me->speedSM_State) {
                case ST_SSMINIT:
                    ...
                    break;
                case ST_SSMWAIT:
                    POST(me, &commControl_processEvent, evSTM, 50, 0);
                    break;
                case ST_SSMSPEED:
                    sendSpeedFrame(me);
                    POST(me, &commControl_processEvent, evADefault, 50, 0);
            ...
            case ST_CSMPROCESS:
                if (Event_getId(ev) == evCMsg) {
                    ...
                    switch (Event_getData(ev)) {
                        case 0x141:
                            me->commSM_State = ST_CSMPOWER;
                            break;
                        case 0x201:
                            me->commSM_State = ST_CSMBRAKE;
                            break;
                        case 0x140:
...
void readCANFrame(CommControl* me, uCAN_MSG* msg) {
    if (CAN_receive(msg) != 0) {
        ...
        CanIdParser cip;
        cip.raw = msg->frame.id;
        uint8_t src = cip.cid.src;
        uint8_t dest = cip.cid.dest;
        uint8_t msgNbr = cip.cid.msgNbr;
        ...
        switch (src) {
            case 0x1:
                switch (msgNbr) {
                    case 0:
                        POST(me, &commControl_processEvent, evCMsg, 0, 0x140);
                        break;
                    case 1:
                        POST(me, &commControl_processEvent, evCMsg, 0, 0x141);
                        break;
                    default:
                        break;
                }
            ...
            case 0x2:
                switch(msgNbr){
                    case 0:
                        POST(me, &commControl_processEvent, evCMsg, 0, 0x201);
                        break;
                    default:
                        break;
                }
