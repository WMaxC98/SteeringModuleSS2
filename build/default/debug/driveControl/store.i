...
void store_init(Store* me) {
    if (DATAEE_ReadByte(EE_INIT) != 0xA5) {
        ...
        DATAEE_WriteByte(EE_ALIVE, 10);
        DATAEE_WriteByte(EE_SPEED, 5);
        DATAEE_WriteByte(EE_STOP, 5);
    }


    me->eeValues[EE_INIT] = DATAEE_ReadByte(EE_INIT);
    me->eeValues[EE_ALIVE] = DATAEE_ReadByte(EE_ALIVE);
    me->eeValues[EE_SPEED] = DATAEE_ReadByte(EE_SPEED);
    me->eeValues[EE_STOP] = DATAEE_ReadByte(EE_STOP);
}
